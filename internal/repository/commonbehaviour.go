...
func (r RedisCommonBehaviour[T]) Get(ctx context.Context, id entity.ID) (T, error) {
	...
	if err != nil {
		...
		logrus.WithError(err).WithField("id", id).Errorln("couldn't retrieve entity from redis")
		return t, err
	}
...
func (r RedisCommonBehaviour[T]) Save(ctx context.Context, ent entity.Entity) error {
	...
	if err := r.client.Do(ctx, cmd).Error(); err != nil {
		logrus.WithError(err).WithField("ent", ent).Errorln("couldn't save entity to redis")
		return err
	}
	return nil
}
